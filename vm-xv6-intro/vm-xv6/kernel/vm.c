...
setupkvm(void)
{
  ...
  // Allocate new page of memory using kalloc() to hold a new directory
...
// Given a parent process's page dir, create a copy
// of it for a child.
pde_t*
...
int mprotect(void *addr, int len){
  ...
  if((uint) addr >= proc->sz || (uint) addr + len * PGSIZE > proc->sz){
    return -1;
  }
